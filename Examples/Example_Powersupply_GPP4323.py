...
myGPP4323.set_current(vcc_ch_num, vcc_current_limit)

# %% ==========================================================================
# Measurement
# =============================================================================
myGPP4323.set_output(channel=vcc_ch_num, state='ON') # turn on the channel or
# myGPP4323.set_all_outputs(state='ON') # turn on all channels

records = [] # Empty list to store data and meta data
for idx in tqdm(range(num_of_points)):
    rec = {} # single record
    rec["VCC_Voltage"] = myGPP4323.measure_voltage(vcc_ch_num)
    rec["VCC_Current"] = myGPP4323.measure_current(vcc_ch_num)
    rec["Power"] = myGPP4323.measure_power(vcc_ch_num)
