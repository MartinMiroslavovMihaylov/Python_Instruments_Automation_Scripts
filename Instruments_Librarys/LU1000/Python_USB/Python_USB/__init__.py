...
class _PathInitializer(object):
    ...
    SUPPORTED_PYTHON_VERSIONS = ['2_7', '3_7', '3_8']
    RUNTIME_VERSION_W_DOTS = '9.10'
    RUNTIME_VERSION_W_UNDERSCORES = '9_10'
    PACKAGE_NAME = 'Python_USB'
    ...
    def read_path_file(self):
        ...
        if length == 4:
            (bin_dir, extern_bin_dir, runtime_dir, ml_dir) = filtered_lines
            if (not os.path.isdir(bin_dir)) or (not os.path.isdir(extern_bin_dir)) or (not os.path.isdir(runtime_dir)) or (not os.path.isdir(ml_dir)):
                return False
            else:
                (self.bin_dir, self.extern_bin_dir, self.runtime_dir, self.ml_dir) = (bin_dir, extern_bin_dir, runtime_dir, ml_dir)
                return True
        else:
            return False
    ...
    def update_paths(self):
        ...
        #For Windows, add the bin_dir and extern_bin_dir to the OS path. This is unnecessary
        #for Linux and Mac, where the OS can find this information via rpath.
        if self.is_windows:
            os.environ[self.path_var] = self.bin_dir + os.pathsep + self.extern_bin_dir + os.pathsep + os.environ[self.path_var]
